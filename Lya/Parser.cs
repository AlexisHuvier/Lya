...
namespace Lya
{
    ...
    {
        private static bool CheckVarExists(Env env, Token varToken)
        {
        ...
        private static bool CheckVarAlreadyExists(Env env, Token varToken)
        {
            if (!env.variables.ContainsKey(varToken.Value))
                return true;
        ...
        public static dynamic Parse(List<Token> tokens, Env env = null)
        {
            ...
            List<List<Token>> expressionsTokens = Utils.SplitTokensOnType(tokens, TokenType.SemiColon);
            ...
            foreach (var expression in expressionsTokens)
            {
                ...
                switch (expression[0].Type)
                {
                    ...
                    case TokenType.KeywordType when expression.Count >= 4 && expression[1].Type == TokenType.Identifier && expression[2].Type == TokenType.Operator && expression[2].Value == "=": 
                        expressions.Add(new VarDeclaration() {VarName = expression[1].Value, Type = Utils.GetVariableTypeFromKeyword(expression[0].Value), Value = Parse(expression.GetRange(3, expression.Count - 3))});
                        break;
